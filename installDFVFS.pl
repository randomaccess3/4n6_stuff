...
# Theres an error when unzipping the files from master.zip, the tests will pass if you extract the one file it fails on using something like winrar
...
use WWW::Mechanize;
use version;
use Archive::Zip qw( :ERROR_CODES ); #requires version 1.57
...
chdir('master\\dfvfs-master\\') or die "$!";
